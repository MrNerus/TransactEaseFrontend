...
export class CardService {
  ...
  transferCards(cardIds: string[], targetOrganizationId: string): void {
  ...
  assignCard(cardId: string, userId: string, issueDate: Date, expiryDate: Date): void {
    this.cards.update(cards =>
      cards.map(card =>
        card.id === cardId ? { ...card, userId, status: 'active', issueDate, expiryDate } : card
      )
    );
  }
